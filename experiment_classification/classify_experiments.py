...
do_train = sys.argv[2].lower() == "train"
...
if do_train:
  ...
  classifier.fit(X, y_inds)
  print >>sys.stderr, "Dumping classifier to classifier.pkl and indexes to indexes.pkl"
  pickle.dump(classifier, open("classifier.pkl", "wb"))
  pickle.dump((word_index, idfs), open("indexes.pkl", "wb"))
else:
  classifier = pickle.load(open("classifier.pkl", "rb"))
  word_index, idfs = pickle.load(open("indexes.pkl", "rb"))
  testfile = codecs.open(sys.argv[1], "r", "utf-8")
  data = []
  for line in testfile:
    text = " ".join(word_tokenize(line.strip())).lower()
    data.append(text)
